...
try
{
	...
	//  xArrow
	auto testCylinder = make_cylinder(true, 128, {0.4f, 0.4f, 0.4f}, make_rotation_z(3.141592f / 2.f) * make_scaling(8.f, 2.f, 2.f));
	GLuint testCylinderVao = create_vao(testCylinder);
	std::size_t testCylinderVertexCount = testCylinder.positions.size();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// CYLINDER
		Mat44f model2world = make_translation({0.f, 0.f, 0.0f});
		...
		GLuint normalMatrixLocation = glGetUniformLocation(prog.programId(), "uNormalMatrix");
		glUniformMatrix3fv(normalMatrixLocation, 1, GL_TRUE, normalMatrix.v);
		...
		// Bind VAO
		glBindVertexArray(testCylinderVao);

		// Draw a triangle (3 Vertices) starting at index 0
		glDrawArrays(GL_TRIANGLES, 0, testCylinderVertexCount);
		// reset state
		glBindVertexArray(0);
		glUseProgram(0);
